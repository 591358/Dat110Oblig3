...
public class FileManager {
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	try {
			...
			for(BigInteger i : replicafiles) {
				...
				successor.addKey(i);
				successor.saveFileContent(filename, i, bytesOfFile, counter == index);
				counter++;
			}
	...
	public NodeInterface findPrimaryOfItem() {
		...
		for(Message messageForPeer : activeNodesforFile) {
			...
			if(messageForPeer.isPrimaryServer()) {
				// return the primary when found (i.e., use Util.getProcessStub to get the stub and return it)
				return Util.getProcessStub(messageForPeer.getNodeName(), messageForPeer.getPort());
			}
		}
		return null;
	}
