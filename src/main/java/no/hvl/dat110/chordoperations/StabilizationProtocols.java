...
public class StabilizationProtocols extends TimerTask {
	...
	private void stabilizeRing() throws NoSuchAlgorithmException {
	 ...
	 */
	private void updateSuccessor() throws NoSuchAlgorithmException {
		try {
			...
			try {
				if(chordnode.getNodeName().equals(predsucc.getNodeName())){
					...
					((Node) chordnode).copyKeysFromSuccessor(predsucc); 										// copy keys from successor
